...
render-disparity fingerprint. Its historical name does not permit an
...
`PipelineOrchestrator` owns stage order and model lifetime. After
`DepthMapProcessor.generate_depth_map_files` returns, its existing `finally`
path releases the depth model. The orchestrator then selects the effective
render-disparity input:
...
The actual call remains exception-safe. VDPP is released in a `finally` block
inside the temporal stage and downstream work begins only after a complete,
validated stabilized stage exists.
...
validation. It is serialized in job settings and accepted by Web and CLI. It
does not require a processing-settings schema bump because old settings files
normalize to the new default `off` without losing information.
...
Web segmented control / CLI flag
  ...
  -> PipelineOrchestrator source selection
...
```json
{
  ...
  "completed_shot_ids": [0],
  "fingerprint": "sha256-of-all-other-fields"
}
```

The example lists only one frame name for readability; the real manifest lists
all selected source frame names. `status` becomes `complete` only when every
shot and every expected payload validates. The fingerprint is
`canonical_json_hash` over every other field, including completion state.

`attention_backend` records the effective upstream attention implementation
(`xformers` or `torch`) rather than an unavailable requested name. The
execution plan is built by the same code that supplies forward-call arguments;
there are no duplicated precision or downsize literals.

A complete stabilized stage describes historical execution provenance and is
reusable even if the current machine would select a different attention
backend: no new neural output is being mixed into it. Resuming a partial stage
requires the current effective attention backend to match the persisted plan;
otherwise the complete stabilized stage is reset before any new shot is
written. Stable algorithm fields such as precision, downsize, window, range
policy, source revision, and checkpoint hash must match in both cases.
...
replaced. A shot ID is appended to `completed_shot_ids` only after all of its
expected PNGs pass name, shape, bit-depth, and count checks. If a process stops
mid-shot, its orphan payloads are not trusted. The next run deletes the files
in that incomplete shot range and recomputes the complete shot.

If a completed shot has a missing or malformed PNG, only that shot is removed
from completion state and recomputed. Because later shots reset temporal state,
they remain internally reusable. Stereo and downstream output are nevertheless
invalidated when the selected stabilized source fails validation; rendering
does not begin from a partially repaired source.
...
A      = bounded model activations for S <= 32 at the fixed working shape
```
...
The model adapter has at most 32 padded inputs, 32 current padded outputs, four
previous padded references, and one finalized resize result resident as
external device tensors around forward/alignment/emission:

```text
69 * Pp + model parameters + A
```

`A` depends on the installed attention kernel, but it is bounded because
sequence length is at most 32 and the working spatial shape is fixed by the
canonical native shape and the pinned downsize transform. Neither expression
contains video length or shot length. Tests instrument decoded, retained, and
device-visible sequence counts rather than merely inspecting the loop.
...
load selected depth estimator
  ...
  -> release depth estimator
  -> load VDPP only when requested and not already cached
...
After strict checkpoint load and before the first stabilized payload is
committed, the adapter runs one inference-mode preflight using a constant 0.5
tensor with:

- sequence length `min(32, longest_shot_length)`;
- the exact canonical native shape;
- the exact padded and downsize transforms;
- FP32 and the effective attention backend.

The constant input exercises the same tensor shapes without depending on a
particular first shot. It is released immediately after the trial. Preflight
failure leaves base canonical output untouched and reports the native shape,
working shape, requested plan, and CUDA OOM cause.
...
The default is `off`. Web and CLI feed the same typed setting. There are no
`vdpp_window`, `vdpp_overlap`, `vdpp_strength`, `vdpp_resolution`, or
`vdpp_precision` controls.
...
non-CUDA device before job creation. For resume, the existing read-only resume
audit may allow the job when a complete exact-match stabilized stage is already
present; otherwise it rejects before pipeline mutation. CLI follows the same
...
- A missing, corrupt, short, or reordered canonical payload fails the owning
  shot with a path/range-specific error.
...
- User cancellation stops at the next loader/window/output boundary, leaves the
  current shot incomplete, releases the model, and preserves prior complete
  shots.
- A requested VDPP failure returns a failed job. It does not silently render
  from base canonical disparity.
- When the setting is off, missing source code or checkpoint is irrelevant: no
  VDPP import, network request, warning, or device allocation occurs.
...
- Test the actual upstream `compute_scale_and_shift` path, including degenerate
  determinant behavior, rather than monkeypatching alignment away.
...
- Test old settings files with no field as exact default-off compatibility.
- Test full stabilized reuse without importing VDPP or resolving its checkpoint.
- Test interruption in a shot, deletion/recompute of only that incomplete shot,
  and reuse of earlier complete shots.
- Test one corrupt payload in a completed shot and downstream invalidation.
- Test source canonical, scene manifest, algorithm, source revision, checkpoint,
  downsize, precision, and range-policy identity changes. Test attention-backend
  mismatch separately: reuse a complete stage, but reset a partial stage before
  producing more shots.
...
- Test early CPU/MPS rejection before download and output mutation when VDPP
  generation is required, plus CPU reuse of a complete exact-match cache.
- Test synthetic maximum-shape CUDA preflight before the first payload commit.
- Test that the depth estimator release callback runs before VDPP construction
  and VDPP release runs before stereo, including exceptions and cancellation.
...
checkpoint's best domain. Before removing the Experimental label:

- benchmark base versus VDPP on Sintel with ground-truth TGSE, AbsRel, and
  delta-1, using the same canonical integration path as production;
- include DA3MONO, DA3METRIC, and the V2 control path where practical;
- require median TGSE to improve and median spatial error not to regress by more
  than 2 percent for every configuration claimed as recommended;
- inspect temporal slit-scans and final stereo output for edge breathing,
  foreground/background swaps, clipping, lag, and over-smoothing;
- run an anime/See-Through clip set separately because VDPP was trained on
  VKITTI, TartanAir, and PointOdyssey depth generated by an image-depth model,
  not on anime;
- keep a backend/domain combination experimental or document it as not
  recommended when it fails, rather than adding hidden backend exceptions.
...
7. Stabilized metadata binds source canonical, scene manifest, source revision,
   checkpoint hash, and the complete effective execution plan.
8. Interrupted or corrupt output cannot be mistaken for a complete stabilized
   stage, and valid complete shots can resume.
...
10. The depth estimator and VDPP are never resident together by orchestration
    design, including failure paths.
...
1. Add the setting, CLI/Web control, dynamic progress plan, and default-off
   compatibility tests without constructing VDPP.
2. Add the shared render-disparity validator and producer-specific stabilized
   metadata contract while keeping current canonical/stereo tests green.
3. Vendor the minimal pinned Apache-2.0 source subset and implement secure,
   lazy checkpoint resolution.
4. Implement the bounded 32/4 shot iterator test-first against the pinned
   upstream reference.
5. Add the file-backed `TemporalDepthStabilizer`, shot-atomic resume, and cache
   invalidation matrix.
6. Insert the stage after depth-model release and before stereo, then verify
   lifetime, cancellation, preflight, and OOM paths.
7. Update architecture, parameters, usage, installation, performance,
...
- `docs/ARCHITECTURE.md`: optional derived disparity stage, source selection,
  and model lifetime order.
- `docs/PARAMETERS.md`: the one setting, default, CUDA requirement, and no
  tunable temporal parameters.
- `docs/USAGE.md`: Web/CLI examples and explicit off-mode recovery after OOM.
