...
are forbidden. Partial-resume runtime identity includes both the NumPy version
and a deterministic behavior probe over this reducer, the correlation fixture,
and a fixed sequential Chan merge. Tests include an odd 17-value float32 tile
whose two-pass M2 differs by one ULP from elementwise Welford, plus empty and
one-value tiles.
...
partial_resume_numeric_runtime_policy =
...
Runtime identity additionally records `python_implementation`, `python_version`,
`platform_system`, `platform_machine`, `sys_byteorder`, `numpy_version`,
`opencv_version`, and `numeric_reducer_probe`. The probe serializes the exact
binary64 results of the pinned scalar/pair reducers and sequential scalar/pair
Chan merges with `float.hex()`. A probe change resets the entire building
artifact even when `numpy_version` is unchanged; a NumPy-version change also
resets it even when the probe matches.

OpenCV is conservatively version-bound for partial resume, but its semantic
contract is the decoded uint16 pixels, not identical PNG compression bytes.
Per-file hashes protect integrity, so no encoder-byte probe participates in
runtime identity. Partial shots generated by a different bound runtime are not
mixed. The algorithm, execution plan, and runtime changes invalidate v1
stabilized artifacts without changing any depth-model setting or base canonical
cache key.
...
8. A 61-frame synthetic shot proves overlap frames enter statistics once,
   ...
   unchanged.
9. Instrumentation proves the host bound above, input normalization holds only
   one full-frame boolean mask and reverses it in place, memmaps close before
   Windows unlink, no owned float64 host array exceeds `STATS_TILE_PIXELS`, and
   disk preflight uses pending shots plus the largest pending memmap.
...
13. Shot diagnostics are self-hashed and strictly validated; threshold, tile
    ...
    invalidate or prevent mixed partial resume as specified. The same NumPy
    version with a different numeric probe and a different NumPy version with
    the same probe both reset a building artifact and reuse no completed shot.
